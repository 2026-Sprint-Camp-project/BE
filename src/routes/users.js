...
app.post("/users/signup", async (req, res) => {
    try{
        ...
        const hashedPassword = await bcrypt.hash(password, 10)      //비밀번호 암호화(slatRounds: 10)
...
app.get("/users/:userId", authenticateToken, async (req, res) => {
    try{
        ...
        //const [[{ isFollower }]] = await pool.query(`SELECT EXISTS (
            //SELECT 1 FROM follow WHERE following_id = ? AND follower_id = ?) AS isFollower`, [userId, myId]);
